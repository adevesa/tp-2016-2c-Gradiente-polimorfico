...
t_socket_server* server_stream_create(int puerto, int backlog)
{
	...
	address_config_in *direccionServidor =configurar_address_in(puerto);
...
void server_modela_nuevo_cliente(t_socket_server *server)
{
	...
	int cliente_new = server_acepta_conexion_cliente(server->socket_asociado);

	if(cliente_new !=0)
	{
		server_aumenta_numero_de_conectados(server, cliente_new);
		server_saluda_al_cliente(cliente_new);
		server_informa_conexion_exitosa(cliente_new);
...
{
	char* msg = "Bienvenido\n";
	enviar_a_socket(socketCliente, msg, strlen(msg));
}
...
void server_atende_cliente(t_socket_server *server, int cliente, int longitudPaquete, char *buffer)
{
	...
	if(result == 0)
		{
			server_cerra_cliente(server, cliente);
		}
}
...
void ejecutar_server_select(int puerto, int backlog, int longpaquete)
{
	//CREACION E INICIALIZACION DEL SERVIDOR//
		...
		while(1)
		{
			...
			//Se llama a SELECT para esperar peticiones:
			//* o bien puede haber una nueva conexión entrante
			//* o uno de los sockets clientes puede enviar un mensaje
			int result;
			//Posibles casos para result:
			// result < 0 --> error
			// result = 0 --> fuera de tiempo
			// result > 0 --> alguien está llamando!
			...
			if(result >0)
			{
				...
				server_stream_atende_clientes(server, readset_temporal, longpaquete);
